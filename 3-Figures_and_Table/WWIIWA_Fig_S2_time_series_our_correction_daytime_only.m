...
% =========================================================================
% Figure S4 - Panel a: R1-3 but daytime-only version
% =========================================================================
figure(141); clf; hold on;
patch([1940.5 1945.5 1945.5 1940.5],[-100 -100 100 100],[.6 .6 .6]+0.2,'facealpha',0.3,'linest','none')
plot_time_series_three_way(Full_day.SST_glb_raw,Bucket_day.SST_glb_raw,ERI_day.SST_glb_raw);
plot_time_series(Full_day.SST_glb_raw,[],[0 0 0],-1,[],1); 

CDF_panel([1930 1955 -.45 .75],'','','','','fontsize',16,'fontweight','normal')
set(gca,'ytick',[-.4:.2:.6])
set(gcf,'position',[.1 13 7 5],'unit','inches')
set(gcf,'position',[5.1 15 5 4.5],'unit','inches')

% =========================================================================
% Figure S4 - Panel b: R5
% =========================================================================
figure(142); clf; hold on;
patch([1940.5 1945.5 1945.5 1940.5],[-100 -100 100 100],[.6 .6 .6]+0.2,'facealpha',0.3,'linest','none')
plot_time_series(Full_day.SST_glb_adj,RND_day.SST_glb_adj,[0 0 0],-1,[],1); 
plot_time_series_three_way(Full_day.SST_glb_adj,Bucket_day.SST_glb_adj,ERI_day.SST_glb_adj);
plot_time_series(Full_day.SST_glb_adj,[],[0 0 0],-1,[],1); 
...
% =========================================================================
% Fiigure S4 - Panel c: Collocated difference but daytime-only version
% =========================================================================
figure(143); clf; hold on;
patch([1940.5 1945.5 1945.5 1940.5],[-100 -100 100 100],[.6 .6 .6]+0.2,'facealpha',0.3,'linest','none')

plot(1930:1955,nanmean(Dif_Bucket_ERI_day.SST_glb_raw,1),'color',[0 0 0]+0.5,'linewi',3); 
plot(1930:1955,nanmean(Dif_Bucket_ERI_day.SST_glb_adj,1),'color',[.7 0 0],'linewi',3); 
...
% =========================================================================
% Figure S4 - Panel d: Groupwise decomposition but daytime-only version
% =========================================================================
...
target_yr = [1934:1951];
pic_raw  = squeeze(nanmean(Full_day.SST_glb_raw(:,target_yr-1929),1));
pic_grp  = squeeze(nanmean(Full_day.SST_glb_adj(:,target_yr-1929),1)) - pic_raw;
pic_idv  = squeeze(nanmean(IDV_day.SST_glb_adj(:,target_yr-1929,1:N_grp),1)) - repmat(pic_raw',1,N_grp);

figure(144); clf; hold on; clear('h')
...
plot(target_yr,-pic_raw-0.25,'w-','linewi',5);

CDF_histplot(target_yr, pic_grp,'-','k',2);
plot(target_yr,-pic_raw-0.25,'-','linewi',2,'color',[1 1 1]*.6);
...
set(gcf,'position',[.1 13 7 5],'unit','inches')
set(gcf,'position',[.1 13 13 2],'unit','inches')
...
% for ct = 1:5
%     file_save = [dir_save,'Fig.S4_',num2str(ct),'.png'];
