% *************************************************************************
% This version removes buoy SSTs in the paring,
% because two months in 2009 takes forever to finish...
% whereas the current analysis does not rely on buoy SSTs.
% 2019-09-26
% 
...
function LME_pair_01_Raw_Pairs_no_buoy(P)
    ...
    if ~isempty(DATA),
        ...
        Markers = [double(DATA.C0_CTY_CRT(l_use,:))  DATA.C1_DCK(l_use)'  Method(l_use)'];

        in_var  = [DATA.C98_UID(l_use); DATA.C0_LON(l_use);  DATA.C0_LAT(l_use);...
                   DATA.C0_UTC(l_use);  DATA.C0_SI_4(l_use)];
        uid_index  = 1;
        lon_index  = 2;
        lat_index  = 3;
        utc_index  = 4;
