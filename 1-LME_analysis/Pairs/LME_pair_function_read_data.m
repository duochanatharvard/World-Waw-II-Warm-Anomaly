...
function output = LME_pair_function_read_data(P)
    ...
    try
        ...
        % *****************************************************************
        if P.yr > 1940 && P.yr < 1946
            if isfield(P,'correct_GB_204_night')
                if P.correct_GB_204_night == 1
                   l_target = ismember(C0_CTY_CRT,'GB','rows') & C1_DCK' == 204 & C1_ND' == 1;
                   C0_SST(l_target) = C0_SST(l_target) - 0.3;
                end
            end
        end   
