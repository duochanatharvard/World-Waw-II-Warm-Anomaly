...
% the following parameters are assigned in the submission pipine
% P.do_nighttime_LME = 1;
% P.exp_bck_id = num;

[WM,ST,NUM] = LME_correct_infer_day_night(P);
...
if ~isfield(P,'only_correct')
    ...
    if P.only_correct == 1
       app_name = 'ERI_infer_US';
    elseif P.only_correct == 0
        switch P.exp_bck_id
            case 1
                app_name = 'bucket_infer_UK';
            case 2
                app_name = 'bucket_infer_JP';
            case 3
                app_name = 'bucket_infer_both';
            case 4
                app_name = 'bucket_no_infer';
        end
    end
    if P.do_individual == 1
        dir_save  = LME_OI('idv_corr');
        file_save = [dir_save,'Corr_idv_method_',num2str(P.only_correct),'_en_',num2str(P.en),'_',P.save_lme,'_',app_name,'.mat'];
    else
        dir_save  = LME_OI('rnd_corr');
        file_save = [dir_save,'Corr_rnd_method_',num2str(P.only_correct),'_en_',num2str(P.en),'_',P.save_lme,'_',app_name,'.mat'];
    end
end
