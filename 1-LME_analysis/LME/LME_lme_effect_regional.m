function group_region = LME_lme_effect_regional(mx,my,reso)

    if reso ~= 5,
        disp(['Though assigned resolution is: ', num2str(reso),' degrees'])
        disp('Computed resolution is fixed to 5 degrees')  
    end
    mask_region = load_regions;
    mask_region(mask_region == 0) = nan;
    group_region = LME_function_grd2pnt(mx,my,[],mask_region,5,5,[]);

end

function mask_region = load_regions
